...
from parser.constants import RNN_EXPT_DIRECTORY, TRIGGER_FN_LABELS_PATH
from parser.model import Model
from parser import utils


class TriggerFunctionModel(Model):
    """Model for predicting Trigger Functions from descriptions.

    This class identifies Trigger Functions by pairing them with their Trigger
    Channels. So, for example, a pure Trigger Function "new_photo_post" for the
    Trigger Channel "Facebook" is interpreted as "Facebook.new_photo_post".
    This is done to differentiate between two Trigger Functions from different
    Trigger Channels with the same keyword.
    """

    def __init__(self, config, path, stem=True):
        super(TriggerFunctionModel, self).__init__(config, path, stem)

    def _create_label_maps(self):
        """Creates mapping from label keywords to ids by loading the mapping
        from appropriate CSV file.
        """
        self._load_label_maps(TRIGGER_FN_LABELS_PATH)
    ...
    def _convert_to_one_hot(self, labels):
        ...
        indices = []
        for label in labels:
            indices.append(self.labels_map[label.trigger_fn])
...
def main():
    ...
    if args.model[0] == "TriggerFunctionModel":
        model = TriggerFunctionModel(configs.PaperConfiguration, path, True)
