...
from parser.constants import RNN_EXPT_DIRECTORY, ACTION_FN_LABELS_PATH
from parser.model import Model
from parser import utils


class ActionFunctionModel(Model):
    """Model for predicting Action Functions from descriptions.

    This class identifies Action Functions by pairing them with their Action
    Channels. So, for example, a pure Action Function "upload_to_drive" for the
    Action Channel "Google" is interpreted as "Google.upload_to_drive".
    This is done to differentiate between two Action Functions from different
    Action Channels with the same keyword.
    """

    def __init__(self, config, path, stem=True):
        super(ActionFunctionModel, self).__init__(config, path, stem)

    def _create_label_maps(self):
        """Creates mapping from label keywords to ids by loading the mapping
        from appropriate CSV file.
        """
        self._load_label_maps(ACTION_FN_LABELS_PATH)
    ...
    def _convert_to_one_hot(self, labels):
        ...
        indices = []
        for label in labels:
            indices.append(self.labels_map[label.action_fn])
...
def main():
    ...
    if args.model[0] == "ActionFunctionModel":
        model = ActionFunctionModel(configs.PaperConfiguration, path, True)
